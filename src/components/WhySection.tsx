        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-5">
          <ScrollReveal animation="slide-in-left">
            <div className="rounded-t-2xl md:rounded-2xl border border-red-500/40 bg-gradient-to-b from-red-950/70 to-black/50 backdrop-blur-md p-6 md:p-8 flex flex-col relative overflow-hidden">
              ...
              <div className="flex items-center gap-4 mb-2 mt-4 md:mt-0">
                <img src="/images/pill-bottle.png" alt="Pill bottle" className="w-16 h-16 md:w-20 md:h-20 object-contain flex-shrink-0 opacity-60 md:opacity-100" loading="lazy" />
                <div>
                  <h3 className="font-heading text-[20px] md:text-[28px] text-white/70 md:text-white drop-shadow-md">The Pill Approach</h3>
                  <p className="font-body text-[14px] text-red-400 uppercase tracking-wide font-semibold">Temporary & Risky</p>
                </div>
              </div>
              <ul className="flex flex-col gap-4 md:gap-5 mt-4 md:mt-6">
                {pillProblems.map(({ icon: Icon, text }) => (
                  <li key={text} className="flex items-center gap-3">
                    ...
                    <span className="font-body text-[17px] md:text-[20px] text-white/60 md:text-white drop-shadow-sm leading-snug">{text}</span>
                  </li>
                ))}
              </ul>
            </div>
          ...
          <ScrollReveal animation="slide-in-right">
            <div className="rounded-b-2xl md:rounded-2xl border-2 border-green-400/40 bg-gradient-to-b from-green-950/40 to-black/50 backdrop-blur-md p-6 md:p-8 flex flex-col relative overflow-hidden shadow-[0_0_30px_rgba(74,222,128,0.1)]">
              <div className="absolute top-0 right-0 bg-cta text-cta-foreground font-body text-[12px] font-bold uppercase tracking-wider px-5 py-1.5 rounded-bl-xl">Better Way</div>
              <div className="flex items-center gap-4 mb-2 mt-4 md:mt-0">
                <img src="/images/bottle-1.png" alt="HeartAide dropper bottle" className="w-16 h-16 md:w-20 md:h-20 object-contain flex-shrink-0" loading="lazy" />
                <div>
                  <h3 className="font-heading text-[20px] md:text-[28px] text-white drop-shadow-md">The HeartAide Approach</h3>
                  <p className="font-body text-[14px] text-green-300 uppercase tracking-wide font-semibold">Natural & Lasting</p>
                </div>
              </div>
              <ul className="flex flex-col gap-4 md:gap-5 mt-4 md:mt-6">
                {heartAideBenefits.map(({ icon: Icon, text }) => (
                  <li key={text} className="flex items-center gap-3">
                    ...
                    <span className="font-body text-[17px] md:text-[20px] text-white drop-shadow-sm leading-snug font-semibold">{text}</span>
                  </li>
                ))}
              </ul>
            </div>
